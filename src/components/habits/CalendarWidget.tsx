...
export default function CalendarWidget({ history, onDayClick, selectedDate }: Props) {
  ...
  for (let i = 0; i < firstDay; i++) {
    ...
    cells.push(
      <div key={`prev-${i}`} style={{ color: "var(--chalk)", fontSize: 13, textAlign: "center", padding: "6px 0" }}>
        {d}
      </div>,
    );
  }
  ...
  for (let i = 1; i <= remaining; i++) {
    cells.push(
      <div key={`next-${i}`} style={{ color: "var(--chalk)", fontSize: 13, textAlign: "center", padding: "6px 0" }}>
        {i}
      </div>,
    );
  }
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <button
          onClick={prevMonth}
          style={{ background: "none", border: "none", fontSize: 18, cursor: "pointer", color: "var(--gravel)", padding: "4px 8px" }}
        >
          ‹
        </button>
        <span style={{ fontSize: 15, fontWeight: 500, color: "var(--obsidian)" }}>
          {year} 年 {month + 1} 月
        </span>
        <button
          onClick={nextMonth}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 0, marginBottom: 8 }}>
        {WEEKDAYS.map((w) => (
          <span key={w} style={{ fontSize: 11, color: "var(--gravel)", textAlign: "center", padding: "4px 0" }}>
            {w}
          </span>
        ))}
      </div>
